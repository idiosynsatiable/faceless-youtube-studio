...
export function buildCaptionTrack(script: string, durationSeconds: number): string {
  ...
  const each = Math.max(1, durationSeconds / groups.length);
  return groups.map((line, index) => {
    const start = index * each;
    const end = Math.min(durationSeconds, Math.max(start + 0.8, (index + 1) * each));
...
async function runFfmpeg(args: string[]): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    const child = spawn(process.env.FFMPEG_BINARY ?? '/usr/bin/ffmpeg', args, { stdio: ['ignore', 'ignore', 'pipe'] });
    let stderr = '';
...
export async function writeNarrationAssets(projectDir: string, script: string, durationMinutes: number): Promise<{ narration: string; captions: string }> {
  ...
  await fs.writeFile(captions, buildCaptionTrack(script, Math.max(60, durationMinutes * 60)), 'utf8');
